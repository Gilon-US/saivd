-- PostgREST matches RPC overloads by argument types; the client sends JSON keys in an
-- order that resolves to (new_role text, target_id uuid). Recreate the function with
-- that signature so `supabase.rpc('set_user_role', …)` is found.
--
-- Also allow the bootstrap owner email (must match `BOOTSTRAP_SUPERUSER_EMAIL` in
-- `src/lib/bootstrap-superuser.ts`) to change roles even when `profiles.role` is still `user`.
