...
import type {SkippedVideoUpload} from "@/hooks/useVideoUpload";

type SkippedVideosReportProps = {
  skipped: SkippedVideoUpload[];
};

export function SkippedVideosReport({skipped}: SkippedVideosReportProps) {
  ...
  return (
    <Alert className="border-amber-200 bg-amber-50 text-amber-950 dark:border-amber-900 dark:bg-amber-950/40 dark:text-amber-50">
      ...
      <AlertTitle>
        {skipped.length} duplicate video{skipped.length === 1 ? "" : "s"} skipped
      </AlertTitle>
      <AlertDescription>
        <ul className="mt-2 max-h-40 overflow-y-auto space-y-1 text-sm">
          {skipped.map((item) => (
            <li
              key={`${item.file.name}-${item.file.size}-${item.reason}`}
